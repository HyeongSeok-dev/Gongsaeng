...
var schedules = [];
	...
	// updateHiddenValue 함수 정의
	window.updateHiddenValue = function(checkbox, hiddenInputId) {
	    var hiddenInput = document.getElementById(hiddenInputId);
	    hiddenInput.value = checkbox.checked ? '1' : '0';
	    // 체크되지 않았을 경우에 숨겨진 입력 필드를 비활성화
...
// -----------------------------------------------------------------
document.getElementById('classRegisterForm').addEventListener('submit', function(e) {
    e.preventDefault(); // 폼 기본 제출 동작 방지

    var formData = new FormData();
    var checkboxes = document.querySelectorAll('input[type="checkbox"][name^="class_day"]');
    ...
    // 이제 formData를 사용하여 AJAX 요청을 보낼 수 있습니다.
});
...
function processScheduleData() {
    ...
    // 숨겨진 필드에서 선택된 요일 처리 (수정 필요)
    $('input[type="hidden"][id$="_value"]').each(function() {
        var dayValue = $(this).val(); // 숨겨진 필드의 값
        var dayId = $(this).attr('id').replace('_value', ''); // 요일 ID 추출
        if (dayValue === '1') {
            var dayIndex = dayStringToIndex($('#' + dayId + '_checkbox').next('label').text().trim());
